...
class Worm:
    def __init__(self, coordinate, body_types, color, worm_brain, map_max=25):
        self.coordinate = coordinate
        ...
        self.puppeteer_time = random.randint(1, 5)
    ...
    def process_egg_laying_phase(self):
        free_places = list(filter(lambda x: self.worm_brain.check_way(x), self.get_possible_egg_places()))
        if len(free_places) > 0:
            egg_place = random.choice(free_places)
            new_worm = Worm(egg_place, self.body_types_origin, self.color, self.worm_brain)
    ...
    def get_possible_egg_places(self):
        ...
        for multiplier in multipliers:
            ...
            result.append(Coordinate(multiplier.x,   multiplier.y+1))
            result.append(Coordinate(multiplier.x+1, multiplier.y+1))
        return list(set(map(lambda x: self.normalise_coordinate(x), result)))
